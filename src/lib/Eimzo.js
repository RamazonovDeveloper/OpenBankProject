import { EIMZOClient as Client } from './components/e-imzo/e-imzo-client'
...
export default class EIMZO {
   ...
   * @return {Promise<SignPkcs7Result>}
   */
  async appendPkcs7Attached(pkcs7_64, id) {
    return new Promise((resolve, reject) => {
      Client.appendPkcs7Attached(
        pkcs7_64,
        id,
        (/* string */ pkcs7) => {
          resolve(pkcs7)
  ...
  async getTimestampTokenRequestForSignature(signature_hex) {
    return new Promise((resolve, reject) => {
      Client.getTimestampTokenRequestForSignature(
        signature_hex,
        (/* string */ pkcs7) => {
          resolve(pkcs7)
        },
