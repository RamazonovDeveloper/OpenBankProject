...
export const EIMZOClient = {
  ...
  appendPkcs7Attached: function (pkcs7_64, id, success, fail) {
    console.log('pkcs7_64', pkcs7_64)
    console.log('id', id)
    CAPIWS.callFunction(
      {
        plugin: 'pkcs7',
        name: 'append_pkcs7_attached',
        arguments: [pkcs7_64, id]
      },
      function (event, data) {
        console.log('appendPkcs7Attached', data)
        if (data.success) {
          success(data)
        } else {
          fail(null, data.reason)
      ...
      function (e) {
        console.log('appendPkcs7Attached error', e)
        fail(e, null)
      }
    )
  },
  getTimestampTokenRequestForSignature: function (signature_hex, success, fail) {
    CAPIWS.callFunction(
      {
        plugin: 'tsaclient',
        name: 'get_timestamp_token_request_for_signature',
        arguments: [signature_hex]
      },
      function (event, data) {
        console.log('DATA TIME STAP', data)
        if (data.success) {
          success(data)
        } else {
          fail(null, data.reason)
  ...
  createPkcs7: function (id, data, timestamper, success, fail) {
    ...
    console.log('data data FILE', JSON.stringify(data))
    CAPIWS.callFunction(
      {
        plugin: 'pkcs7',
        name: 'create_pkcs7',
        arguments: [Buffer.from(JSON.stringify(data), 'utf8').toString('base64'), id, 'no']
      },
      function (event, data) {
        console.log('data data', data)
        console.log('timestamper', timestamper)
        if (data.success) {
