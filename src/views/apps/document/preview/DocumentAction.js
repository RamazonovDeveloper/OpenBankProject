...
const DocumentAction = ({ id, toggleSendInvoiceDrawer, toggleAddPaymentDrawer, data, getInfo, isScrolledToBottom }) => {
  ...
  return (
    <Box sx={{ display: 'flex', gap: '20px' }}>
      ...
      <LoadingButton
        className={isScrolledToBottom ? 'main_btn' : 'main_btn '}
        style={isScrolledToBottom ? { backgroundColor: '#4E0F8A', fontSize: '17px', fontWeight: 600 } : { backgroundColor: '#4E0F8A', fontSize: '17px', fontWeight: 600, opacity: '0.5' }}
        onClick={() => putHash()}
        loading={loading}
